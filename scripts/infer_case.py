...
from physicsai_gnn.inference import predict_once
...
def main():
    ...
    field, scalar = predict_once(model, data, device, normalizers)
    field_names = names_for_prediction(data, target_field_arrays(cfg), field.shape[-1])

    result = {
        "global_features": global_features,
        "field_names": field_names,
        "field_prediction_shape": list(field.shape),
    }

    if scalar is not None:
        result["scalar_names"] = target_scalar_columns(cfg)
        result["scalar_prediction"] = scalar.reshape(-1).tolist()
